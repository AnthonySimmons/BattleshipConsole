 ...
 * Description:	This function prints the welcome screen along*
 *			with the game rules and the symbols used in the	 *
 *			game.											 *
 * Input parameters: None									 * 
 * Returns: Nothing							                 *
 * Preconditions: None										 *
 * Postconditions: Allows the user to understand how the game*
 *			works.										 	 *
 *************************************************************/
 ...
 * Input parameters: Two endpoints for each ship(x1,y1,x2,y2)* 
 *			each player's board (both two dimensional arrays)*
 *			integer that decides whether or not to print the *
 *			board, player number, target coordinates, number *
 *			of hits, and character that represents each ship *
 * Returns: The game board					                 *
 * Preconditions: Must have all the variables listed above   *
 * Postconditions: Allows the program to initialize the game *
 *			board, place the ships on the board, and update	 *
 *			the board after each shot. It also allows the 	 *
 *			user to see the game board.						 *
 *************************************************************/
 ...
 *			player goes first.								 *
 * Input parameters: None									 * 
 * Returns: 'player' (which player starts first)             *
 * Preconditions: None										 *
 * Postconditions: Allows the program to decide whether	     *
 *			player1(You) or player2(CPU) goes first.		 *
 *************************************************************/
...
/*************************************************************
 * Function: deploy_ships()									 *
 * Description:	This function prompts the user to decide	 *
 *			whether to place their ships randomly or manually*
 * Input parameters: None									 * 
 * Returns: 'deploy' (whether its random or manual)          *
 * Preconditions: None										 *
 * Postconditions: Allows the program to decide whether to   *
 *			deploy the ships randomly or to prompt the user  *
 *			for position of each ship.						 *
 *************************************************************/
 ...
 * Input parameters: The addresses of the endpoint			 *
 *			coordinates (x1, y1, x2, y2), the size of the	 * 
 *			ship, and the array representing player1's board *
 * Returns: Nothing but modifies the endpoint values         *
 * Preconditions: Must have the input parameters and the game*
 *			board initialized								 *
 * Postconditions: Allows the program to place player1's     *
 *			ships on the board if the endpoints entered fit	 *
 *			the ship and don't overlap other ships	 		 *
 *************************************************************/
 ...
 *			ship.											 *
 * Input parameters: The addresses of x1, y1, x2, y2		 *
 *			the size of the ship, and the player's board	 *
 * Returns: Nothing, but modifies the values of endpoint     *
 *			coordinates										 *
 * Preconditions: Must have the player's board intitialized	 *
 * Postconditions: Allows the program to randomly place ships*
 *			on the board(s), for player2 and player1 if		 *
 *			'deploy' = 2. This program only returns the		 *
 *			endpoint coordinates when the ships position fits*
 *			the board, is not diagonal, and does not overlap.*
 *************************************************************/
...
/*************************************************************
 * Function: shoot_manual()									 *
 * Description:	This function prompts the user to enter the	 *
 *			target coordinates (xtarget, ytarget)			 *
 * Input parameters: The addresses of xtarget and ytarget,	 * 
 *			and player2's board.							 *
 * Returns: Nothing, but modifies the values of the target   *
 *			coordinates.									 *
 * Preconditions: Must have player2's board initialized		 *
 * Postconditions: Allows the user to shoot, and allows the  *
 *			program to decide whether the shot is a hit or a *
 *			miss, so long as the user has not already shot	 *
 ...
 * Input parameters: xtarget, ytarget, player(#),hit(or miss)*
 *			the addresses of the sunk values for each ship	 *
 *			(sunkc, sunkb, sunkr, sunks, sunkd)				 *
 * Returns: Nothing, but modifies the values of each ships   *
 *			sunk status										 *
 * Preconditions: Must outfile created and initialized		 *
 * Postconditions: Allows the user to see the shot info for  *
 *			each shot, from the 'Battleship.log' file		 *
 *************************************************************/
 ...
 * Input parameters: *outfile, player(#), hits, misses,		 * 
 *			shots, and accuracy 							 *
 * Returns: Nothing, just prints to file					 *
 * Preconditions: Must have outfile created and initialized	 *
 * Postconditions: Allows the user to see the game statistics*
 *			for each player after the game is over.			 *
 *************************************************************/
